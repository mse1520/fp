...
function ipFormatter(value) {
  ...
  return _go(value, _replace(/[^(0-9|.)]/g, ''), _split('.'), _map(_regexMatch(/25[0-5]|2[0-4][0-9]|[0-1]?[0-9]?[0-9]/g)), _forEach(v => v?.length > 1 ? overFlowFlag = true : undefined), _map(_head), _map(v => v ? v : ''), v => overFlowFlag && _last(v) !== '' ? _push(v, ['']) : v, _take(4), _join('.'));
}
